...
# Comandos de carga e execução do YCSB para MongoDB
LOAD_COMMAND="/app/ycsb-0.18.0/bin/ycsb.sh load mongodb -s -P workloads/workloada -P db.properties -p hosts=${MONGODB_NODES} -p port=${MONGODB_PORT} -P replica.properties"
RUN_COMMAND="/app/ycsb-0.18.0/bin/ycsb.sh run mongodb -s -P workloads/workloada -P db.properties -p hosts=${MONGODB_NODES} -p port=${MONGODB_PORT} -P replica.properties"
